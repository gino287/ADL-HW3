...
argparser.add_argument("--result_file_name", type=str, default="result_norerank.json")
...
BATCH_Q = 4096 # you can adjust this value to fit hardware constraints
BATCH_GEN = 64 # you can adjust this value to fit hardware constraints
TEST_DATA_SIZE = 300  # set to -1 to run on the whole test set 
...
for b_start in tqdm(range(0, len(tests), BATCH_Q)):
    ...
    # 5) skip reranker, directly use FAISS results

    # 6) recover each query, calculate MRR@K, create generation prompt
    messages_list = []
    rerank_info_list = []
    for b, (q, (low, high)) in enumerate(zip(queries, idx_slices)):
        if low == high:  # 如果這個 query 沒有候選 passage
            MRR_sum += 0.0
            N += 1
        ...
        # 直接沿用 FAISS 的檢索順序，不重排
        reranked = list(zip([0.0]*len(cand_ids), cand_ids, cand_text))
        ...
        # 選取前 TOP_M 個段落給 LLM
        context_list = cand_text[:min(TOP_M, len(cand_text))]
        messages = [
            {
                "role": "system",
        ...
        rerank_info_list.append([
            {"pid": pid, "text": text, "score": 0.0}
            for pid, text in zip(cand_ids, cand_text)
        ])
